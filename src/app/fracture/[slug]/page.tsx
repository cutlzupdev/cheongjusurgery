...
export async function generateStaticParams() {
  return getConditionsByCategory("fracture").map((c) => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const condition = getCondition("fracture", slug);
  if (!condition) return {};
  return {
...
export default async function FractureConditionPage({ params }: Props) {
  const { slug } = await params;
  const condition = getCondition("fracture", slug);
