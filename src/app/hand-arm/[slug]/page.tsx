...
export async function generateStaticParams() {
  return getConditionsByCategory("hand-arm").map((c) => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const condition = getCondition("hand-arm", slug);
  if (!condition) return {};
  return {
...
export default async function HandArmConditionPage({ params }: Props) {
  const { slug } = await params;
  const condition = getCondition("hand-arm", slug);
