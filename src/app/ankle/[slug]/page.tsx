...
export async function generateStaticParams() {
  return getConditionsByCategory("ankle").map((c) => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const condition = getCondition("ankle", slug);
  if (!condition) return {};
  return {
...
export default async function AnkleConditionPage({ params }: Props) {
  const { slug } = await params;
  const condition = getCondition("ankle", slug);
