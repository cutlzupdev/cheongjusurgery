...
export async function generateStaticParams() {
  return getConditionsByCategory("foot").map((c) => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const condition = getCondition("foot", slug);
  if (!condition) return {};
  return {
...
export default async function FootConditionPage({ params }: Props) {
  const { slug } = await params;
  const condition = getCondition("foot", slug);
