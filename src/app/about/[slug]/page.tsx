...
export async function generateStaticParams() {
  return getConditionsByCategory("about").map((c) => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const condition = getCondition("about", slug);
  if (!condition) return {};
  return {
...
export default async function AboutConditionPage({ params }: Props) {
  const { slug } = await params;
  const condition = getCondition("about", slug);
