...
export async function generateStaticParams() {
  return getConditionsByCategory("sports-rehab").map((c) => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const condition = getCondition("sports-rehab", slug);
  if (!condition) return {};
  return {
...
export default async function SportsRehabConditionPage({ params }: Props) {
  const { slug } = await params;
  const condition = getCondition("sports-rehab", slug);
