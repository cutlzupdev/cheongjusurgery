...
export async function generateStaticParams() {
  return getConditionsByCategory("shoulder").map((c) => ({ slug: c.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const condition = getCondition("shoulder", slug);
  if (!condition) return {};
  return {
...
export default async function ShoulderConditionPage({ params }: Props) {
  const { slug } = await params;
  const condition = getCondition("shoulder", slug);
